...
return r.toString();}
function a_crr(a){var b='/assets/embed/cientifica-aj-';switch(a){case'hlp_f':b+='fin.html';break;case'hlp_t':b+='trig.html';break;case'hlp_e':b+='est.html';break;case'hlp_o':b+='out.html';break;}
...
function tmp_msg(){jQuery('#LBSubResu').html('Insira um valor');setTimeout(function(){jQuery('#LBSubResu').html('');},2000);}
function pcalc(op,id,v){switch(modo){case"A":var x=(v=='d')?'':'m';if(jQuery('#'+x+id).hasClass("uk-open")!=true){var i=parseInt(id.replace('b',''));var r=assprf.replace(/_n_/g,i);jQuery('#'+x+'dp'+id).html(r+abtns[i]);}
break;case"C":if(uFunc!=''&&uFunc!=op)lmpt();if(jQuery('#TIExp').val()=='')tmp_msg();else{calc_c(op);chgfnl();}
break;case"M":var i=parseInt(id.replace('b',''));if(abtns[i]!=null){var x=(v=='d')?'':'m';if(jQuery('#'+x+id).hasClass("uk-open")!=true){var r=assprf.replace(/_n_/g,i);jQuery('#'+x+'dp'+id).html(r+abtns[i]).promise().done(function(){});}}else{if(uFunc!=''&&uFunc!=op)lmpt();if(jQuery('#TIExp').val()=='')tmp_msg();else{calc_c(op);chgfnl();}}
break;}}
...
function mudamodo(md){jQuery('.trmais').remove();switch(md){case'A':jQuery('#painelConv').hide('fast');jQuery('.dpdw_').addClass('uk-dropdown');if(carregado("/assets/calc/ctf-a.min.js")==false){var scp=document.createElement('script');scp.src="/assets/calc/ctf-a.min.js";document.getElementsByTagName('head')[0].appendChild(scp);}
rjpt('ctf-c.min.js');rjpt('ctf-m.min.js');break;case'C':jQuery('#painelConv').show('fast');jQuery('.dpdw_').html('');jQuery('.dpdw_').removeClass('uk-dropdown');if(carregado("/assets/calc/ctf-c.min.js")==false){var scp=document.createElement('script');scp.src="/assets/calc/ctf-c.min.js";document.getElementsByTagName('head')[0].appendChild(scp);}
rjpt('ctf-a.min.js');rjpt('ctf-m.min.js');break;case'M':jQuery('#painelConv').hide('fast');jQuery('.dpdw_').html('');jQuery('.dpdw_').removeClass('uk-dropdown');jQuery('.dpdm').addClass('uk-dropdown');if(carregado("/assets/calc/ctf-m.min.js")==false){var scp=document.createElement('script');scp.src="/assets/calc/ctf-m.min.js";document.getElementsByTagName('head')[0].appendChild(scp);}
...
function dpf(x){var p,a,b;p=(x+'').split('.');a=p[0];b=p[1];return(b==undefined)?'Erro':simp(a+''+b,Math.pow(10,b.length));}
...
function mdc(ar){var c,comm,cont,fat,ver,veri_,d,verif_,veri,verif,resu,fica;ar=ar.split(',');fica=false;for(var i=0;i<ar.length;i++){if(parseInt(eval(ar[i]))<=0)fica=true;}
if(fica==true)return'NaN';c=2;comm=new Array();cont=true;fat=new Array();ver=false;while(cont){veri_=true;for(var i=0;i<ar.length;i++){d=parseInt(eval(ar[i]))%c;if(d!=0)veri_=false;}
